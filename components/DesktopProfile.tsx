...
// DesktopProfile — the profile as a two-column desktop layout. Left column is
// identity + quick links + referral + sign-out; right column is stats and
// achievements. Reuses the same data as ProfileScreen (fills-derived stats,
// referral summary). Rendered inside DesktopShell; mobile ProfileScreen stays.
...
export default function DesktopProfile({ onOpenSettings, onOpenPortfolio, onOpenHistory, onOpenLeaderboard, onOpenAdvanced }: DesktopProfileProps) {
    const { t } = useLanguage();
    ...
    const copyCode = async () => {
        if (referralCode && await copyToClipboard(referralCode)) {
            setCopied(true);
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.15fr)', gap: 20, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div className="v2-card" style={{ padding: 24, borderRadius: 20, textAlign: 'center' }}>
                    <div style={{ width: 84, height: 84, borderRadius: '50%', margin: '0 auto 14px', background: V2.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: V2.accentInk, fontSize: 38 }}>{initial}</div>
                    <button onClick={onOpenSettings} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: V2.ui, color: V2.t1 }}>
                        <span style={{ fontSize: 23, fontWeight: 800, letterSpacing: '-0.02em' }}>{displayName}</span>
                        <Icon name="pencil" size={15} color={V2.t3} />
                    </button>
                    <button onClick={copyAddress} style={{ display: 'block', margin: '4px auto 0', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 13.5, color: addressCopied ? V2.pos : V2.t3, fontFamily: V2.mono }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
                    {quickLinks.map((q) => (
                        <button key={q.label} onClick={q.onClick} className="v2-card" style={{ padding: '16px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7, cursor: 'pointer', color: V2.t1, fontFamily: V2.ui }}>
                            <Icon name={q.icon} size={19} color={V2.accent} />
                            <span style={{ fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>{q.label}</span>
                        </button>
                    ))}
                </div>
                ...
                <button onClick={() => logout()} style={{ width: '100%', padding: 14, borderRadius: 14, background: V2.negSoft, border: '1px solid rgba(239,68,68,0.22)', color: V2.neg, fontWeight: 700, fontSize: 14.5, cursor: 'pointer', fontFamily: V2.ui, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                    <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 12 }}>{t.screens.perfil.section.stats}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        {statCells.map((s) => (
                            <div key={s.l} className="v2-card" style={{ padding: 16, borderRadius: 14 }}>
                                <div style={{ fontSize: 12, color: V2.t3, fontWeight: 600 }}>{s.l}</div>
                                <div style={{ fontSize: 24, fontWeight: 800, marginTop: 4, color: s.c || V2.t1, letterSpacing: '-0.02em', fontFamily: V2.ui }}>{s.v}</div>
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                        <span style={{ fontSize: 15, fontWeight: 800 }}>Logros</span>
                        <span style={{ fontSize: 13, color: V2.t3, fontWeight: 600, fontFamily: V2.mono }}>{achievements.filter((a) => a.on).length}/{achievements.length}</span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                        {achievements.map((a) => (
                            <div key={a.l} style={{ padding: '18px 10px', borderRadius: 14, textAlign: 'center', background: a.on ? V2.accentSoft : 'rgba(255,255,255,0.02)', border: a.on ? '1px solid rgba(227,179,76,0.2)' : `1px solid ${V2.hair}`, opacity: a.on ? 1 : 0.5 }}>
                                <div style={{ fontSize: 28, marginBottom: 6, filter: a.on ? 'none' : 'grayscale(1)' }}>{a.e}</div>
