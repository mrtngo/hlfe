...
export function NewsFeed({ height = 600, showHeader = true }: NewsFeedProps) {
    return (
        <div className="bg-[#0D0D0D] border border-white/10 rounded-2xl overflow-hidden">
            {showHeader && (
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                    ...
                    <a
                        ...
                        className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg text-xs text-coffee-medium hover:text-[#FFFF00] hover:bg-white/10 transition-all"
                    >
            ...
            {/* Info Banner */}
            <div className="px-4 py-3 bg-[#FFFF00]/5 border-b border-[#FFFF00]/20">
                ...
                {NEWS_ACCOUNTS.map((account) => (
                    ...
                    >
                        <div className="flex items-start justify-between mb-2">
                            <div className="w-10 h-10 bg-[#FFFF00]/20 rounded-full flex items-center justify-center">
                                <Twitter className="w-5 h-5 text-[#FFFF00]" />
                            </div>
                            <span className="text-[10px] px-2 py-0.5 bg-white/10 rounded-full text-coffee-medium">
                                {account.category}
                            </span>
                        </div>
                        <h3 className="font-bold text-white text-sm group-hover:text-[#FFFF00] transition-colors">
                            @{account.name}
                        </h3>
                        <p className="text-xs text-coffee-medium mt-1 line-clamp-2">
                            {account.description}
                        </p>
                        <div className="flex items-center gap-1 mt-2 text-[10px] text-[#FFFF00] opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="p-4 border-t border-white/10 space-y-2">
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-3 bg-[#FFFF00] text-black font-bold rounded-xl hover:bg-[#FFD700] transition-all"
                >
